...
2 — відповідь містить **характеристику** і коментаря, що вона означає або як допомагає клієнту у задачі
...
"""
    ...
    Підтримуй контекст заперечення. Будь конкретним, відповідай одним-двома реченнями."""
            ...
            try:
                ...
                full_history = "\n".join([f"Раунд {i+1}: {reply}" for i, reply in enumerate(session["seller_replies"])])
                evaluation_prompt = f"""
        Ти — експерт з продажів. Оціни відповіді продавця на заперечення клієнта.
        ...
        Критерії оцінки:
        1. Чи відповідає на конкретне заперечення?
        2. Чи містить корисну інформацію?
        3. Чи пропонує рішення?

        Шкала:
        - "повністю переконливо" (10) - ясна відповідь з прикладами
        - "частково переконливо" (7) - відповідь по темі, але без доказів
        - "слабкі відповіді" (3) - відповідь не пов'язана з проблемою
        - "некоректні відповіді" (0) - спам або відсутність відповіді

        Відповідай одним словом: "повністю переконливо", "частково переконливо", "слабкі відповіді" або "некоректні відповіді".
        """
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
                    messages=[
                ...
                print(f"[DEBUG] GPT response: {raw_rating}")

                # Визначаємо відповідність оцінки
                rating_mapping = {
                    "повністю переконливо": 10,
                    "частково переконливо": 7,
                    "слабкі відповіді": 3,
                    "некоректні відповіді": 0
                }
                
                # Знаходимо перший збіг у тексті відповіді
                objection_score = 0
                rating = "некоректні відповіді"  # Значення за замовчуванням
                
                for key in rating_mapping:
                    if key in raw_rating:
                        rating = key
                        objection_score = rating_mapping[key]
                        break
