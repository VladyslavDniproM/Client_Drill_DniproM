...
# --- Stage 4: Обробка заперечень ---
    ...
    ФОРМАТ:
    АНАЛІЗ_ВИЯВЛЕННЯ_ПОТРЕБ: [короткий аналіз якості питань]
    ПОРАДИ_ВИЯВЛЕННЯ_ПОТРЕБ: [конкретні рекомендації]
    """
    ...
    ФОРМАТ:
    АНАЛІЗ_ПРЕЗЕНТАЦІЇ: [короткий аналіз якості презентації]
    ПОРАДИ_ПРЕЗЕНТАЦІЇ: [конкретні рекомендації]
    """
                ...
                - **Відповідність запереченню (0-3 бали)**: Чи відповідає продавець безпосередньо на заперечення?
                - **Аргументація (0-3 бали)**: Чи наводить конкретні переваги, факти, приклади?
                - **Емпатія та розуміння (0-2 бали)**: Чи виявляє розуміння проблеми клієнта?
                - **Закриття (0-2 бали)**: Чи пропонує рішення або переводить до покупки?
                ...
                ФОРМАТ ВІДПОВІДІ:
                АНАЛІЗ_ЗАПЕРЕЧЕННЯ: [детальний аналіз за критеріями]
                ПОРАДИ_ЗАПЕРЕЧЕННЯ: [2-3 конкретні рекомендації]
                ...
                rating_match = re.search(r"ОЦІНКА:\s*(ідеально|гарно|стандартно|сумнівно|погано)", evaluation_text, re.IGNORECASE)
                ...
                rating = rating_match.group(1).lower() if rating_match else "погано"
                ...
                answers_score = min(sum(a["score"] for a in session.get("user_answers", {}).values()), 10)  # Змінити з 6 на 10
                objection_score = session.get('objection_score', 0)
                total_score = model_score + questions_score + answers_score + objection_score

                print("\n=== ФІНАЛЬНИЙ РАХУНОК ===")
                print(f"[SCORE] За модель: {model_score}/4")  # Оновити
                print(f"[SCORE] За питання: {questions_score}/8")
                print(f"[SCORE] За відповіді: {answers_score}/10")  # Оновити
                print(f"[SCORE] За заперечення: {objection_score}/8")  # Оновити
                print(f"[SCORE] ЗАГАЛЬНИЙ БАЛ: {total_score}/30")  # 8+4+10+8=30
